...
across subjects of `r label_range[1]`--`r label_range[2]`%).  That is, listeners
...
subjects' boundaries), distributional learning leads to the same shift in
...
was intended to be a /b/ or a /p/.

This is somewhat surprising, given that in the extremely shifted 30ms /b/, 70ms
/p/ condition most of the tokens from the /b/ distribution would normally be
classified as /p/ under a typical talker's generative model.


# Model

The results of Experiment 2 rule out one possible explanation for the
constraints on distributional learning observed

One way to interpret the findings of Experiment 2 is that listeners manage to
identify the statistical structure of the two VOT clusters _without_ labels
telling them which cluster each VOT belongs to.  If they _didn't_, then
providing labels would add extra information which should change their
distributional learning, and this didn't happen.
...
I make a number of simplifying assumptions for the sake of tractability.  First,
and most importantly, I treat the learning as _supervised_, and provide the
model with the mean and standard deviation of the two VOT distributions.  This
...
large number of possibilities to consider.  
