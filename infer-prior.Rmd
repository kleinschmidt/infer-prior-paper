...
Together, these parameters suggest that listeners' prior beliefs about the VOT
distributions associated with /b/ and /p/ are strong, and moreover that their
beliefs in the _variance_ of VOTs is stronger than their beliefs in the
_means_.  In turn, this suggests that the distributional learning observed in
Experiment 1 is driven mostly by changes in listeners' beliefs about the
underlying means, with only small contributions from changes in the variance
(e.g., a relaxation in their standards for what counts as a good /b/ or /p/).


The fact that the prior confidence in
the mean ($\kappa_0$) is lower than the variance ($\nu_0$) indicates that
listeners distributional learning is better described by a _shift_ in the
underlying distributions 


As in previous work with similar belief updating models, the posterior
distribution of these parameters was multimodal, where one mode puts more
confidence in the means ($\kappa_0 > \nu_0$) and adapts mostly by updating
beliefs in the variance, while the other puts more in the variances ($\nu_0 >
\kappa_0$) and adapts by shifting.  The "shifting" solution is preferred, with a
Bayesian $p$ value of `r p_shift`
...
leads to interpretable parameters [@Kleinschmidt2015].  It also imposes strong
