...
```{r confidence-params, fig.width=5.6, fig.height=4.4, fig.caption="Sampled values of prior confidence parameters from belief updating model fit to Experiment 1.  Upper left shows values where $\kappa_0 < \nu_0$ (\"shifting\"), and lower right shows values where $\nu_0 < \kappa_0$ (\"expanding\").  Red stars show approximate MAP values for each kind of solution (individual sample with the highest posterior)."}
...
prior_samples_df %>%
  ...
  scale_y_log10(TeX("Pseudocount for mean ($\\kappa_0$)")) +
  scale_x_log10(TeX("Pseudocount for variance ($\\nu_0$)")) +
  coord_equal() +
  geom_abline() +
...
[^normal-prior]: Specifically, the prior distribution of the mean $\mu$ is
    _conditional_ on the variance, $\mu | \siɡma^2 \sim \mathrm{Normal}(\mu_0,
    \sigma^2 / \kappa_0$.
