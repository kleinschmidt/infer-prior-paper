...
the wide range of shifts used here.
...
`R` [@RCoreTeam2017].[^expt1-formula]  This approach simultaneously estimates the group-level
...
violins/points, replotted from Figure -@fig:expt1-boundaries) distributional
...
1 and 2?  First, the process of belief updating means that at any given point a
...
Second, one prediction of the ideal adapter is that listeners' prior
...
sampled_prior_lhood_200 <- prior_samples_df %>%
  semi_join(samples) %>%
...
bind_rows(mutate(map_prior_lhood, source = "Belief updating\nMAP estimate"),
  ...
  geom_line(size=2) +
  geom_line(data = sampled_prior_lhood_200,
            aes(group=interaction(.draw, category), alpha=exp(lp__ - max(lp__))),
            color=1) +
