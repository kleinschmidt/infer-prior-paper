...
and most importantly, I provide the model with the mean and standard deviation
of the two VOT distributions.  This substantially simplifies the complexity of
the learning problem: in order to model _unsupervised_ learning, a Bayesian
model needs to average predicted behavior over distributional learning given
each possible way that all the tokens could be jointly classified, weighted by
the probability of that classification.  In an experiment with 200 trials, each
of which could be either /b/ or /p/, this ammounts to more than $10^60$
possibilities, an impossibly large number of possibilities to consider.  Even
the approximations that are available are not 
