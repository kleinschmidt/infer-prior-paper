...
```{r expt2-bounds, fig.cap='Category boundaries from Experiment 2'}

expt2_bounds_bysub %>%
  filter(block==3) %>%
  ggplot(aes(x=vot_cond, y=vot, fill=vot_cond, color=vot_cond)) +
  geom_violin(data= . %>% filter(supervised=="unsupervised"), alpha=0.5, color=NA)+
  geom_violin(data= . %>% filter(supervised=="supervised"), alpha=0)+
  ## geom_beeswarm(cex=2) +
