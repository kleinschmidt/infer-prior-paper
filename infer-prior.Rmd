...
```{r exp2-label-dists, fig.cap='Distribution of labeled and unlabeled trials in the "mixed" and "supervised" conditions of Experiment 2'}
...
I recruited `r n_total_2 ` subjects via Amazon's Mechanical Turk.  As in
Experiment 1, subjects were paid $2.00 for participation.  Exclusion criteria
were the same as Experiment 1, and data from `r n_excluded_2` subjects were
excluded from analysis: `r n_subj_repeat_2` for repeated participation
(including subjects who had participated in Experiment 1), `r n_subj_bad_2` for
...
across subjects of `r label_range[1]`--`r label_range[2]`%).

Second, as wipth Experiment 1 I assessed listeners' distributional learning via
category boundaries estimated with a Bayesian logistic mixed-effects model.
This model included data from _both_ Experiments 1 and 2 in order to directly
compare assess potential effects of supervised learning on the strength and time
course of learning.  This model included fixed effects for supervision
condition, all fixed effects from the Experiment 1 model, and all interactions
thereof.  The random effects structure was identical to Experment 1.  Because
listeners' responses were so systematically determined by the label on labeled
trials, _only_ unlabeled trials were included in the model (all trials from
Experiment 1, and half of the trial from Experiment 2).
...
violins/points) distributional learning.  As in @Fig:expt1-bounds, the
...
the presence of three outlier subjects with unreliable responses which leads to
very shallow classification functions and very high estimated boundaries
(greater than 50ms).  These subjects weren't so unreliable as to be excluded by
the pre-set exclusion criteria so they are included in the analysis here for the
sake of completeness.  Even so, there is still no "significant" effect of
supervision in any regression coefficient _or_ the estimated boundary (all
Bayesian $p>0.05$, and the single-subject boundary distribution for this
condition (violins in @Fig:expt2-bounds) are very similar.

## Discussion

The results of Experiment 2 suggest that the unsupervised nature of
distributional leanring is _not_ the primary source of the limitations observed
in Experiment 1.  Surprisingly, the average (fixed effects) category boundaries
observed from distributional learning with 50% labeled trials are statistically
indistinguishable from , as are the the
