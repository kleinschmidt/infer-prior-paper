...
The results of Experiment 4 differ in at least two important ways from the
results of Experiments 1 and 2.  First, there was less distributional learning
overall.  Second, the particular pattern of distributional learning across
conditions in this experiment was not predicted by the same combination of prior
beliefs and Bayesian belief-updating that explained the patterns of learning
from Experiment 1.

There are a number of possible reasons for these discrepancies.  First, the
distributions that listeners were exposed to in this experiment differed in
important ways from the earlier experiments.  First, they deviated even more
...
little distributional learning was observed.  This is consistent with
@Sumner2011, who also found that negative VOTs failed to produce large shifts in
category boundaries except in a few particular presentation orders.  However,
the lack of robust distributional learning for large negative VOTs contrasts
with the results from Experiments 1 and 2.  Those experiments found that even
when distributional learning was incomplete (compared with the ideal boundaries
implied by the exposure distributions), there were still robust differences in
VOT classification compared with other exposure distributions.  But in
Experiment 4, the differences between the /b/-/p/ boundaries in the conditions
with negative VOTs did not differ reliably from each other.
...
of the test phase gradually disappear, see Supplementary Table -@tab:exp4-reg).
...
* these results suggest a *continuous* view of the relationship between the kind
  of learning that happens during language acquisition and the kind of learning
  that happens when adults are adapting to an unfamiliar talker.
* primary difference is the amount and type of prior experience that learners
  can draw on.
* moreover, learning from that prior experience is, in some sense, also a form
  of distributional learning, where people must learn not only the distributions
  of *sounds* but the distribution of *accents* (or of phonetic category
  distributions themselves).
* in this view, the primary difference between language acquisition and
  adaptation comes from the continuous operation of distributional learning at
  multiple levels: at least the levels of phonetic categories (acquiring the
  linguistic structure of the language) and accents or generative models
  (acquiring the *indexical* or *social* structure of the language environment)


However, a final Experiment 4 suggests that the constraints on distributional
learning may be more complex than can be captured by the simple belief updating
model of Experiment 3.

In the remainder of this paper, I will discuss the implications of these results
for future work, and finally the caveats suggested by Experiment 4.


## Implications for future work


- overall implication: suggests a continuous operation of a distributional
  learning mechanisms through acquisition and adult language use.
    - at some (computational) level both adaptation and acqusition ARE
      distributional learning
    - all distributional learning has to start somewhere
    - where you start from can dramatically affect the outcome of your learning:
      if your prior expectations are too vague learning will take too long, but
      if they're too specific then you might be wrong.  ideally, want to
      probabilistically match your prior expectations to the distribution of
      things that you will actually encounter.
...
acquisition and adult language use and adptation, and the primary difference
between acquisition and adaptation is simply different amounts and types of
prior experience that the system can draw on.  The idea that statistical
...
a belief updating model, the listener must have _learned_ something about the
patterns of variability across talkers.  At a computational level [in the sense
of @Marr1982], this may also be a form of distributional learning: in order to
effectively adapt to an unfamiliar talker's accent, a listener needs to have
some reasonably good estimate of the amount and kind of talker variability they
should expect, which is directly related to the distribution of talkers' accents
that exist in their environment [@Kleinschmidt2015b].

<!-- also: evidence that babies are engaged in indexical-level learning from an -->
<!-- early age as well.  sensitive to indexical information -->


<!-- blah blah: -->
If their prior expectations are too vague, then

That is, constraints on learning
...
This raises two related sets of questions for future research: (1) what
distributions are *available* for learning, and (2) do listeners' distributional
learning biases and constraints reflect the distributions across talkers that
they have experienced?

### What distributions are available to learn?

Chodroff, LCN paper, etc...

Bozena, Schertz: do these differ by language?  does native language background
affect adaptation/DL BEYOND just the structure of the language (e.g., which
categories do you already have?  what cues?)
...
and so any of the measurements we can make in experiment provide only indirect
evidence about those prior expectations.  Some kind of linking hypotheses are
needed in order to be able to assess these implicit prior expectations.

One of my hopes at the beginning of this project was that the kind of belief
updating model presented in Experiment 3, along with data on the degree and
...
subjective prior beliefs.  In principle, this is a straightforward application
...
to recover them from behavior.  This procedure did yield some meaningful
information (beyond the simple "existence proof" of the suitability of
belief updating as a model for distribtuaion learning).  First, the model tells
...
specfic expectations about the mean and variance of the VOT distributions for
...
like, since they depend on all of the various assumptions that the model makes
in linking behavior to the cognitive variables of interest.  These assumptions
can be fundamental to the logical structure of the model, but they can also be
...
would be colored by our own assumptions about what VOT distributions should look
like, negating somewhat the usefulness of this technique for "mind reading".
...
the many linking hypotheses behind this model the experimental data is
incorrect.  In the following section, I discuss a number of reasons why this
model may have failed to generalize (in addition to the reasons discussed in
Experiment 4 itself).

## Is distributional learning really belief updating?
...
they are compatible with the _theory_ of Bayesian belief-updating.  They just
correspond belief updating models which make different assumptions about the
...
one where there is some amount of drift or change over time even _within_ a
...
initially more rapid and assymptote more quickly than this particular belief
...
variation within a talker, in addition to global variation across talkers.
<!-- 
is this right?  look at production data...relates to the stuff that Shannon
Heald has done.  more broadly, relates to the issues of distributional learning
the prior.
-->
