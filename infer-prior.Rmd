...
measured during exposure in the 10, 50ms condition from Experiment 1 (outline
violin).  On the one hand, this valides the use of the post-test procedure in
Experiment 4 as a way of measuring the outcome of distributional learning.  On
the other hand, the greater variability in these measurements suggests that the
use of a post-test might reduce the power to detect _differences_ in learning
between conditions of Experiment 4.
...
boundary_diff_p_vals <-
  ...
  filter(block == 1) %>%
  spread(vot_cond, vot) %>%
  summarise(p_20_50 = mean(`-20, 50` > `-50, 50`),
            p_50_80 = mean(`-50, 50` > `-80, 50`),
            p_20_80 = mean(`-20, 50` > `-80, 50`))
...
category boundaries, these differences are not very reliable (Bayesian $p = 
`r boundary_diff_p_vals$p_20_50`$ for -20ms vs. -50ms, $p = 
`r boundary_diff_p_vals$p_50_80`$ for -50ms vs. -80ms) except for largest such
difference between the -80ms and -20ms condition (Bayesian $p=
`r boundary_diff_p_vals$p_20_80`$).



There are two notable differences with the results from Experiment
1 (Figure -@fig:exp1-results).  First, there appears to be more between-listener
variability in the classification functions in each condition, especially for
the conditions with large negative VOTs.  This may correspond to differences in
the procedure (
...
<!-- RE lack of learning in high VOT conditions -->
Note, though, that because the separation between the high and low VOT clusters
in these conditions was so large, using the typical talker's boundary would
result in very few errors.
