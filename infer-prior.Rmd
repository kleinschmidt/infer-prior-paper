...
distributions.  On the other hand, there was _some_ distributional learning even
for the most shifted distributions in Experiments 1 and 2, whereas in this
...
In this experiment, listeners were exposed to VOT distributiosn that differed in
...
Moreover, the belief updating model assumes that talkers' productions can be
characterized as two VOT clusters.  However,
...
A belief-updating model that does not know the true cluster for each observed
VOT would likely assign large negative VOTs to the pre-voiced cluster [see
e.g. @Kleinschmidt2015b eq. 7, p. 155], leading to minimal change in beliefs
about the un-aspirated cluster and hence minimal changes in categorization.

Testing this in an implemented model presents a number of technical challenges.
...
cluster labels should be when there are three clusters (two for for phonemically
...
ideal boundary for a "typical talker" would yield low error rates (less than `r
max_error_perc` in all conditions; see Figure -@fig:exposure-dists-exp4).  Thus
...
<!-- TODO: is this really necesary or helpful? -->
...
different VOT distriubtions in Experiment 4.  Does this mean that the
...
<!-- TODO: something a bit more optimistic here? -->
