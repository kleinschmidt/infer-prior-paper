...
important ways from the earlier experiments.  First, some conditions included
large negative VOTs, for which little distributional learning was observed.
This is consistent with @Sumner2011, who also found that negative VOTs failed to
produce large shifts in category boundaries except in a few particular
presentation orders.  The lack of robust distributional learning for large
negative VOTs contrasts with the results from Experiments 1 and 2.  Those
experiments found that even when distributional learning was incomplete compared
with the boundaries implied by the exposure distributions, it was still robust
compared with other exposure distributions.  But in Experiment 4, the
differences between the /b/-/p/ boundaries in the conditions with negative VOTs
...
learned from the earlier bimodal distributions.

Even though the 10ms /b/, 50ms /p/ condition showed a similar degree of learning
as in Experiment 1, this is the condition where the exposure and test ranges
have the most overlap.  Moreover, the differences between the conditions
gradually disappear over the course of the test block, especially for the most
extreme (-80ms, 50ms and 10ms, 80ms VOT) conditions (see Supplementary table
@REF)
