...
distributional learnign [@Feldman2013b; @Feldman2013], it is much weaker than
...
often provide a great deal information about the intended category for a
...
Reliable classification is defined as accuracy of at least 80% at 0 and 70ms
...
`R` [@RCoreTeam2017].  This approach simultaneously estimates the group-level
...
and individual-level effects [@Gelman2007].[^shrinkage] The advantage of using a
Bayesian approach in this case, over standard mixed-effects models as fit with
...
what intended to be two separate clusters of VOTs---one for /b/ and
...
may improve the degree of learning, or the speed, or both.
...
condition of Experiment 1 was run after analyzing data from the other conditions
...
```{r exp2-sup-effects, eval=FALSE}
b_logit_exp2 %>%
  gather_draws(`b_.*supervised.*`, regex=TRUE) %>%
  group_by(.variable) %>%
  mutate(p_gt_zero = mean(.value > 0)) %>%
  ggplot(aes(x=.value, y=.variable, fill=qlogis(p_gt_zero))) +
  geom_halfeyeh() +
  scale_fill_gradient2(mid = gray(0.9), limits=c(-4, 4))
```
...
the design of Experiment 4 required a change in procedure (which I discuss
next).
...
[^why-not-beginning]: The predictions from a regression model are more uncertain
    ...
    like trial number. Thus I use estimated value at the trial that is one-sixth into the post-test to estimate
...
next).  As in Experiment 1, subjects were excluded ($n=`r n_excl4 %$% sum(n)`$)
...
learning of voice onset time (VOT) as a cue to word-intial stop voicing is
...
for future work, and finally the caveats given a final Experiment 4, which
...
distributions across talkers that they have experienced?
...
belief updating as a model for distribtuaion learning in this experiment).  First, the model tells
...
strongly from a typical talker.  However, Experiment 4 suggests that there may
be additional (or different) constraints on distributional learning of VOT
distributions for voicing in American English.  First, distributional learning
was overall _weaker_ in many conditions than in Experiments 1 and 2, despite
larger shifts in the VOT distributions.  Second, the belief updating model from
Experiment 3 (trained on learning behavior from Experiment 1) did a poor job of
predicting the degree of distributional learning observed in Experiment 4.

Above, I discussed a number of differences between the experimental procedures
themselves between Experiments 1 and 4, which may explain some of the observed
differences.  Here, I will discuss more general assumptions of the model which
may also explain these diffrences

### Potential points of failure in the belief updating model

Comparing the belief updating model from Experiment 3 with the data from
Experiment 4 is a stringent test, and many hypotheses and assumptions would be
need to be correct (or close enough) to generalize successfully.  That is, this
comparison tests not only whether the overall belief updating hypothesis
correct, but also that the particular belief updating _model_ which instantiates
this hypothesis is a sufficiently close approximation to the actual processes
that listeners are engaging during distributional learning.  The failure of the
...
one where there is some amount of drift, variabiltiy, or change over time even _within_ a
