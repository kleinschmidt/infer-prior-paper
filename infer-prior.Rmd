...
expt1_bounds <-
  ...
  arrange((Estimate - 0.5)^2) %>%
  filter(row_number() <= 2) %>% #arrange(bvotCond,block)
  # now have two closest points (vot, Estimate)
  summarise(vot = (0.5 - Estimate[1]) * diff(vot)/diff(Estimate) + vot[1])
...
expt1_bounds_bysub %>%
  ...
  geom_hline(yintercept=prior_bound, color="black", linetype=2) +
