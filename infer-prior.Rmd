...
The first way to evaluate the model is how well it fits the data, compared to
various possible baseline models.  For this purpose I use a method which
approximates leave-one-out cross-validation to provide an estimate of the
out-of-sample prediction error from the posterior samples of the model
[PSIS-LOO, @Vehtari2017].  This provides a goodness-of-fit measurement that
penalizes overly complex models in a way that is directly (approximately)
related to their inability to generalize beyond the data they were fit to, and
as such is a more robust measure for comparing between different models than
metrics like AIC or BIC which rely on counting numbers of parameters [see
@Vehtari2017 for more discussion].  Because data was nested within subject, I
used subjects as the cross-validation units (instead of individual trial
responses).
