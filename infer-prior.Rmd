...
datset, there is uncertainty associated with them (and the differences between
...
The belief updating model can be evaluated in a number of ways.  At the most
basic level, it quantiatively fits the data well, as shown by Table -@tbl:loo-tab.
The LOOIC for the model is better (lower) than the LOOIC of two baseline models:
conclusively so for the null, intercept-only model model, and numerically better
for the more sophisticated baseline model, although the SE of that difference is
large so it is not possible to say conclusively.[^aic-uncertainty] Still, the
