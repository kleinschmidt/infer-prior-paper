...
and /p/ that are most likely (maximum a posteriori/MAP) given the distributional
learning behavior.  The belief updating model recovers distributions that are
...
similar variance, while /b/ has a similar mean but higher variance.
...
starting from a shared set of prior beliefs.
...
Finally, can the prior beliefs inferred based on the pattern of distributional
learning from Experiment 1 explain the distributional learning or lack thereof
observed in this experiment?  Figure -@fig:plot-exp4-predictions-first-third
shows the posterior predictive intervals for the belief updating model's
classification functions compared with the average probability of /p/ responses
in the first third of the test phase of Experiment 4.  The model's predictions
are quite accurate for the 10ms /b/, 50ms /p/ condition which is a near
replication of the same exposure condition from Experiment 1.  But for the other
conditions, the model tends to predict *more* distributional learning (larger
boundary shifts) than listeners actually show, or in the case of the two most
extreme negative VOT conditions, makes very vague predictions.


## Discussion

The results of Experiment 4 suggest that some caution is due in generalizing
from Experiments 1 and 2.  In particular, while the results of Experiment 1 are
consistent with belief updating, the prior beliefs inferred as most likely based
on those exposure distributions do not generalize well to distributional
learning to the distributions used in this experiment.  These distributions
differed in two important ways from those of the earlier experiments.  First,
they included large negative VOTs in some conditions, which @Sumner2011 found
failed to produce large shifts in category boundaries except in a few particular
presentation orders.  Similarly, little distributional learning was observed in
conditions with large negative VOTs in this experiment.  

Second, the means of the high and low VOT clusters
(implicitly /b/ and /p/) moved together in lock-step in Experiments 1 and 2,
while in Experiment 4 they were manipulated independently.
...
distributions when asked to rate the goodness or typicality of VOTs. [@CITE]
