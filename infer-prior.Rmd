...
ggplot(mapping=aes(x=vot, y=lhood, linetype=category, color=source)) +
  geom_line(data = map_prior_lhood, aes(color="Belief updating model"), size=1) +
  geom_line(data = prior_lhood, aes(color="Kronrod et al. (2016)"), size=1) +
  geom_line(data = sampled_prior_lhood,
            aes(group=interaction(.draw, category), alpha=exp(lp__ - max(lp__)),
                color="Belief updating model"),
            show.legend = FALSE) +
