...
<!-- I think the interesting thing here is that on the one hand, there's not a -->
<!-- lot of learnign even though these distributions diverge quite a lot from -->
<!-- what I've been calling the "typical" distributions.  on the other hand, for -->
<!-- the negative VOTs they're really not all that different from what someone -->
<!-- might normally produce and so don't really REQUIRE updating.  -->

<!-- the other interesting thing is whether this is consistent with teh story -->
<!-- I've been telling about dist learning being constrained based on prior -->
<!-- experience: the distributions diverge a lot so there should be strong -->
<!-- cosntraints so not a lot of learning. OR, distributions are consistent and -->
<!-- so don't REQUIRE a lot of learning. -->

<!-- more: reasons we might have observed relatively low levels of -->
<!-- distributional learning: -->
<!-- 1. learned dists but doesn't affect classification (three clusters?) -->
<!-- 2. so different from typical talker that didn't believe dists -->
<!-- 3. did learn but "un-learned" during test phase -->

<!-- none of these seems like it can be the whole story...(1) doesn't explain -->
<!-- why you don't see much learning in the 10/80 condition (although maybe -->
<!-- there's something there with limited benefit to shifting boundary?), (2) -->
<!-- doesn't square with the belief updating model not predicting the -->
<!-- constraints (but maybe that's just because the range of dists tested in the -->
<!-- first experiment is too narrow), (3) doesn't square with the fact that you -->
<!-- get very similar boundaries for the 10/50 condition.-->

On the one hand, the results from Experiment 4 are qualitatively consistent with
the earlier experiments: distributional learning is constrained, and the minimal
...
distributions.  On the other hand, the quantitative predictions of a Bayesian
belief updating model---based on the pattern of learning observed in Experiment
1---do not predict the limited learning observed here.  This suggests that
either there are other constraints involved, that there are important
methodological differences between these studies, that the belief updating
hypothesis is wrong, or that technical assumptions made in formulating the
belief-updating model are be wrong.
...
The results of Experiment 4 suggest that, on the one hand, distributional
learning is constrained by prior experience with other talkers, consistent with
Experiments 1, and 2.  On the other hand, the particular prior beliefs
that the simple belief updating model 


...reasonable assumptions mathematically but seems like they don't reflect the
actual constraints.  mismatch between assumptions OR between exposure dists
...
of distributional learning.  The mismatch between the
belief updating model's predictions and the results of Experiment 4 could be due
to technical assumptions made in the modeling process, rather than the belief
updating theory per se.

One suspect assumption that the model makes is that talkers' productions can be
characterized as two VOT clusters, which are normal distributions.  However,
...
the model, and is reasonable given not only that the bimodal structure of the exposure
distributions is extremely clear, but also that Experiment 2 found that providing
cluster labels to many tokens led to no change in distributional learning.
