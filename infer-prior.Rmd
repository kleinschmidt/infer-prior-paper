...
ones [but see @Feldman2013b; @Feldman2013]. Adults have a great deal of
circumstantial evidence from the lexicon, pragmatic context,
...
learning problem of adaptation at least semi-supervised.
...
```{r expt2-class-funs, fig.cap='Classification functions from semi-supervised distributional learning', fig.width=8.5, fig.height=3}
...
```{r inferred-beliefs, fig.width = 7.4, fig.height=5.3, fig.cap="Inferred prior beliefs from belief updating model, compared with prior beliefs estimated by @Kronrod2016.  Inferred priors are derived using the maximum a posteriori (MAP) estimate of the expected prior mean and variance parameters $\\mu_0$, $\\sigma^2_0$."}
...
bind_rows(mutate(map_prior_lhood, source = "Belief updating\nMAP estimate"),
          mutate(prior_lhood, source = "Kronrod et al. (2016)")) %>%
  ggplot(aes(x=vot, y=lhood, linetype=category, color=source)) +
  geom_line() +
...
a posteriori/MAP).  The belief updating model recovers distributions that are
...
A further complication comes from the fact that the belief updating model
assumes that there are exactly two clusters, one for voiced /b/ and the other
for voiceless /p/.  In reality, word-initial voicing in American English is
characterized by _three_ distinct clusters: aspirated (positive VOT) /p/,
unaspirated (small or zero VOT) /b/, and prevoiced (negative VOT) /b/
[@Goldrick2013; @Lisker1964].  This means that listeners are likely to classify
negative VOTs as /b/ even though an "ideal listener" model that has a
low-variance /b/ and high-variance /p/ would (counter-intuitively) classify
those negative VOTs as /p/.  This fact, combined with the equal-variance
clusters produced during experimental exposure, make it highly unlikely that the
belief-updating model would infer the correct asymmetry in variance between /b/
and /p/.

### Strength of prior beliefs

The belief updating model also suggests that these prior beliefs are relatively
strong, with listeners updating their beliefs as if they were based on
$2-5\times$ more VOT samples prior than they would encounter during the
experiment itself.  This is in stark contrast to the results of a similar belief
updating model of recalibration [@Kleinschmidt2015b], which found that listeners
have relatively _weak_ prior beliefs.  That model was mathematically the
same---updating Normal-Inverse $\chi^2$ prior beliefs about cue
distributions---and the number of trials in the experiments were similar (about
200), but @Kleinschmidt2015b found prior pseudocounts on the order of 10s of
observations, rather than 100s like the current study.  Why this discrepancy?
One possibility is that @Kleinschmidt2015b fit that model to data from a
phonetic recalibration study, where listeners repeatedly hear stimuli chosen to
be acoustically ambiguous between /aba/ and /ada/, paired with a label of some
kind of label, like a visual cue to the articulation [@Bertelson2003;
@Vroomen2007].  Experiment 2 found that providing labeling information did not
appreciably change the speed or completeness of distributional learning, but
it's possible that the targeting of specifically ambiguous stimuli rather than
the wider range of stimuli used here could explain the difference [although the
fact that exposure to prototypical stimuli leads to equally rapid belief
updating suggests this is unlikely; @Kleinschmidt2015b].

Another posibility, proposed by @Kleinschmidt2015b, is that the strength of
...
attributed to uncertainty due to the lack of labeling information (Experiment
2), and that the observed constraints are at least consistent with belief
updating starting from a shared set of prior beliefs (Experiment 3).  One
interpretation of the modeling results from Experiment 3 is that the prior
beliefs that the model infers as the starting point for distributional learning
actually correspond to the real contents of listener's prior beliefs.  This
final experiment serves as a stronger test of this idea, by testing
distributional learning with an even wider range of exposure distributions.
The results of this study suggest that the modeling results from Experiment 3
should be interpreted with some caution, and more specifically suggests that the
ability of a belief updating model to provide insight into listeners'
prior beliefs is determined in equally large part by the assumptions of the
model and the structure of variation in the exposure distributions that provide
input to the model.

There are two concrete reasons why the inferred prior beliefs from the belief
updating model might not reflect listeners actual prior beliefs, or even provide
a particularly good account of their behavior in slightly different
distributional learning circumstances.  The first is that American English word
initial stop voicing is actually characterized by _three_ clusters: positive
VOTs (aspirated) which are perceived as /p/, VOTs of approximately zero, and
negative VOTs (prevoicing), which are both perceived as /b/ [@Lisker1964;
@Baese-Berk2009].  The belief updating model assumes there are only two
clusters, and so any large negative VOTs would result, in the model, in large
negative shifts in the low-VOT cluster (which starts near zero, according to the
results of Experiment 3).  However, if listners actually use a three-component
model, then negative VOTs might be assigned to a prevoiced cluster, leaving the
un-aspirated cluster relatively unchanged and little to no change in boundary.
That is, the two-component model may over-estimate the extent to which
exposure to a distribution with large negative VOTs would cause American English
listeners to adjust their categorization boundary.

The second reason is that the model assigns independent prior beliefs to /b/ and
/p/, whereas in reality there is _structure_ in the way these categories' VOT
distributions vary across talkers [@Chodroff2017].  If listeners expect
correlated variation in, for example, the mean VOT of /b/ and the mean VOT of
/p/ across talkers, then there may in fact be _stronger_ than predicted
constraints on learning pairs of distributions that deviate from the expected
pattern of correlation (e.g., higher-than-expected /b/ but lower-than-expected
/p/).  This possibility is exacerbated by the fact that the variation in the two
VOT cluster means were perfectly correlated across conditions in Experiment 1,
since the difference between the high and low VOT clusters was 40ms in every
exposure condition.

To address both of these, Experiment 4 examines distributional learning to
bimodal VOT distributions where the /b/ and /p/ distributions do not vary in
lock-step as in Experiment 1 and 2, and moroever feature a larger range of both
negative and positiev VOTs across conditions.
...
```{r exposure-dists-exp4, fig.width=10, fig.height=3, fig.cap="In Experiment 4, each subject heard a talker that produced one of these five VOT distributions. The variance of each category was constant across conditions, but the means varied semi-independently."}
...
sepmeans %>%
  ...
  labs(title="Experiment 4: VOT distribution conditions",
       subtitle="Mean VOTs for /b/ and /p/ clusters manipulated separately")
...
the design of Experiment 4 required a change in procedure (which we discuss
next).
...
50ms in 10ms steps. This additional phase was necessary because of the large
...
functions, estimated with a hierarchical Bayesian logistic regression model.  In
addition to VOT, a predictor for trial was also included, in order to account
for any (un-)learning effects that might happen during post-test.  In order to
...
next).  As in Experiment 1, we excluded subjects ($n=`r n_excl4 %$% sum(n)`$)
who failed to classify words with unambiguous VOTs reliably during exposure (not
post-test, see above).  After exclusion, `r n_clean4 %$% sum(n)` subjects
remained for analysis (no subjects had previously participated in one of our VOT
studies).  Subjects were evenly distributed across conditions (range of 
...
```{r sepmeans-results, fig.width=10, fig.height=3, fig.cap="Results from experiment 4" }
...
measured during exposure in the 10, 50ms condition from Experiment 1 (empty
violin).  There is also more uncertainty associated with the estimates of the
population-level fixed-effects boundaries as well (the filled vs. open white
...
between the boundaries is more reliable but still small in absolute terms, only
...
However, this lack of robust distributional learning is not unique to large
...
boundaries of only `r boundary_diff_p_vals[['mean_ci']][4]`.
...
Finally, can the prior beliefs inferred from Experiment 1 explain the
distributional learning or lack thereof observed in this experiment?  Figure
-@fig:plot-exp4-predictions-first-third shows the posterior predictive intervals
for the model comparedwith the average probability of /p/ responses in the first
...
The results of Experiment 4 suggest that some caution is due in generalizing
from Experiments 1 and 2.  Those experiments did reveal constraints on
distributional learning, using a range of distributions which varied in how
similar they were to a typical talker's VOT distributions.  However, even though
some of these distributions were quite different from a typical talker, the
range of variation was still rather limited.  The exposure distributions from
this experiment differed in two important ways from those of Experiments 1
and 2.  First, some conditions included large negative VOTs, for which little
distributional learning was observed.  This is consistent with @Sumner2011, who
also found that negative VOTs failed to produce large shifts in category
boundaries except in a few particular presentation orders.
...
Second, the means of the high and low VOT clusters (implicitly /b/ and /p/)
moved together in lock-step in Experiments 1 and 2, while in this experiment
they were manipulated independently.  Varying the means of each cluster together
maximizes the necessity of a boundary shift, since failure to shift the boundary
would result in a large increase in the rate of errors given the implied
category assignments from the clusters.  Exposure distributions where only *one*
cluster deviates from listeners expectations does not *require* as large of a
shift in the boundary between /b/ and /p/ in order to achieve a low recognition
error.  In fact, for all the exposure distributions tested in Experiment 4,
continuing to use the ideal boundary for a "typical talker" would yield low
error rates (less than `r max_error_perc` in all conditions; see Figure
-@fig:exposure-dists-exp4).

The results from this experiment thus suggest that there are further constraints
on listeners distributional learning than were revealed in Experiments 1 and 2.
However, interpreting these constraints requires some caution, since 
...
Together, these results suggest further constraints that guide distributional
learning above and beyond those that are evident from Experiments 1 and 2.


<!-- Is this incompatible with belief updating?  No, just need to 1. take into -->
<!-- account possible correlations in the prior and 2. consider the possibility -->
<!-- that listeners maintain beliefs about the prevoiced cluster and are -->
<!-- updating that instead of the short-lag cluster, which the model doesn't -->
<!-- capture. -->



This lack of robust distributional learning for large negative VOTs contrasts
with the results from Experiments 1 and 2.  Those experiments found that even
when distributional learning was incomplete compared with the boundaries implied
by the exposure distributions, it was still robust compared with other exposure
distributions.


<!-- RE lack of learning in high VOT conditions --> Note, though, that because
the separation between the high and low VOT clusters in these conditions was so
large, using the typical talker's boundary would result in very few errors.
Nevertheless, listeners do usually show sensitivity to shifts in the mean of VOT
distributions when asked to rate the goodness or typicality of words with
varying VOTs. [@CITE]


# General Discussion

In this paper, I have explored the nature and possible explanations of
constraints on distributional learning in adults.  Experiment 1 shows that
distributional learning of voice onset time (VOT) as a cue to word-intial stop
voicing is constrained: listeners learn some distributions ("accents") better
than others, as measured by the agreement between their actual /b/-/p/ category
boundary and the boundary that is implied by the distributions they are exposed
to.  Moreover, listeners show more complete learning for distributions that are
closer to what a typical talker of American English produces, which suggests
that listenres are drawing on their previous experience with other talkers to
...
However, adding labeling information to half the trials had no effect on
distributional learning.  Experiment 3 shows that a belief updating
model---where all listeners start from shared prior beliefs---can capture the
...
In order for prior experience with talker variability to constrain adaptation in
a belief updating model, the listener must have _learned_ the distribution of
accents they have encountered in the past, and hence may expect to encounter in
the future future [@Kleinschmidt2015b].  That is, constraints on learning
distributions of _cues_ (e.g., VOT) in adults may result from distributional
learning at another level, where the observations are _talkers' accents_ (e.g.,
VOT distributions) instead of individual tokens.  Thus, in this way of looking
at it, the primary difference between _adaptation_ and _acquisition_ comes down
to the continued operation of the same kind of distributional learning
mechanisms, at multiple levels.

However, while the results of the first three experiments in this paper support
the idea that distributional learning during adaptation is constrained by prior
experience with other talkers, a final Experiment 4 suggests that the
constraints on distributional learning may be more complex than can be captured
by the simple belief updating model of Experiment 3.  In the remainder of this
paper, I will discuss what this means for the overall conclusion I have been
arguing for.



* suggest that DL is constrained by prior experience with other talkers.
* if this is the case, then these constraints---a primary difference between DL
  in adaptation and acquisition---are just DL at another level: learning a good
  prior for what kinds of accents (VOT distributions) you expect to encounter,
  and using that to guide adaptation.

## Do constraints on distributional reflect prior experience

Without the data from Experiment 4, the evidence from Experiments 1-3 seems to
only support the idea that constraints on distributional learning arise from
...
distributions of a typical talker and those of the exposure talker.  Providing
labeling information does not seem to aid in learning distributions that deviate
strongly from a typical talker.

### Potential points of failure in the belief updating model

Most problematically for the hypothesis that constraints on distributional
learning come from prior experience with other talkers, the prior beliefs
inferred from listeners' distributional learning in Experiment 1 do not
successfully predict the distributional learning in Expeirment 4.  However, this
tests not only whether the prior beliefs hypothesis correct, but also that the
particular belief updating _model_ which instantiates this hypothesis is a
sufficiently close approximation to the putative belief updating processes that
listeners are engaging during distributional learning.  The failure of the
belief updating model (and the prior beliefs inferred from it) to generalize to
learning of a different set of distributions in Experiment 4 could reflect the
failure of assumptions made in the modeling process as well.

One suspect assumption that the model makes is that talkers' productions can be
characterized as two clusters of VOT, which are normal distributions.  However,
even though American English has two phonemic voicing categories, many talkers
produce three distinct _clusters_ of VOTs, with a single long-lag VOT cluster
for voiceless (aspirated) stops, and two clusters for voiced/unaspirated stops:
...
@Goldrick2013].  In an ideal listener model with three clusters, the short-lag
cluster would largely determine where the voiced/voiceless response boundary
falls, since the pre-voiced and short-lag clusters are both phonemically voiced.
After exposure to large negative VOTs, a belief updating model with three
clusters would not update beliefs about the short-lag clusters, and hence
predict little change in the voiced/voicess response boundary.  Testing this in
an implemented model presents a number of technical challenges.  Not least among
them is the fact that the model presented here assumes that the cluster labels
of each token are known.  This assumption massively simplifies the inference in
the model, and is reasonable given that the bimodal structure of the exposure
distributions is extremely clear and that Experiment 2 found that providing
cluster labels for half the tokens led to no change in distributional learning.
But it's unclear what the cluster labels should be when there are three clusters
(two for for phonemically voiced stops), so I leave exploring this possibility
for future work.

The other major architectural assumption of the belief updating model which may
...
Another possible explanation for the failure of the belief updating model to
generalize to Experiment 4 is that a different procedure was used to measure
distributional learning.  Experiment 1 did not distinguish between an exposure
and test phase, and measured distributional learning via responses to trials
during the bimodally distributed exposure.  In Experiment 4, the VOT clusters in
the exposure distributions were so widely separate that in many conditions no
trials had VOTs near the expected boundary, and so a separate post-test phase
was necessary to measure listeners' categorization functions.  In this phase,
listeners experienced a very different _distribution_ of VOTs which was flat
between -10ms and 50ms.  Thus, it's possible that distributional learning
continued throughout the test phase, undoing some of what listeners had learned
from the earlier bimodal distributions.  The presence of a control condition
with the same bimodal distribution as one of the conditions in Experiment 1
(10ms /b/, 50ms /p/) provides _some_ confidence that unlearning was not
complete, but this is also the condition in Experiment 4 with the largest
overlap between the exposure and test distributions, and so it's possible that
the effect of unlearning is larger in the other conditions, where the belief
updating model also failed to generalize.
...
rather drammatic changes in context [e.g. @Theodore2019; @Kraljic2006;
@Kraljic2007], while others find that listeners seem to weight more recent
evidence more strongly, even within a single context [@Saltzman2017;
@Olasagasti2018].  Both of these are compatible with Bayesian belief-updating
_in principle_, they just correspond to different assumptions about the
...
stable _within_ talkers, then each token should be treated as equally
informative.  However, if the generative process is instead one where there is
some amount of drift or change over time even _within_ a context, then this
corresponds to a belief updating model that weights more recent observations
more heavily, since they are assumed to come from distributions with parameters
that are more similar to the current ones.
...
(Figure -@fig:plot-modeled-category-boundaries)......


* Yes: 
    * category boundaries are a compromise between typical talker and exposure
      talker
    * belief updating model of experiment 1
    * inferred prior beliefs look pretty close to typical talker
* No: 
    * belief updating model fails to generalize to experiment 4
    * inferred prior beliefs aren't EXACTLY the same as typical talker (equal
      variance, confidence higher than actual variability)
    * learning curves might be the wrong shape (sharper at the start, flatter at
      the end) and more recent data might count for more [@Saltzman2017;
      @Olasagasti2018]
* Maybe
    * failure of belief updating model could reflect failures of a number of
      assumptions: 
        * 2 categories (whereas there's actually three CLUSTERS that
          are assigned to two phonological categories).
        * use of independent, conjugate prior is very restrictive and might not
          be able to capture the actual variation across talkers
    * different test procedure in Experiment 4: measure with a flat distribution
      from -10 to 50, which doesn't overlap very much with the exposure
      distributions in the more extreme conditions and hence may drive further
      distributional learning.  tried to correct for that in the regression
      analysis but that's impossible to do completely.  and even though the
      replication of the 10,50 condition from expt 1 suggests that it's not that
      different, the range of VOTs included in the test phase overlaps a lot
      with the exposure distributions there so it's least likely to drive
      un-learning of any of the conditions.
    * learning may flatten off and assymptote with more exposure, reflecting
      discounting of less recent trials (Dave and Emily's paper, Giraud
      paper, ).  This is still _compatible_ with belief updating, it just
      suggests that listeners are assuming a different implicit generative
      model, which allows for the fact that a talker's distributions may
      drift, or change slowly, making recent observations more informative
      about the _current_ distributions than are observations from many
      minutes, hours, days, etc. ago.
    * limited range of variation in the exposure distributions for experiment 1
      (so it's possible that there's a compromise set of parameters that
      captures both)

## Can adaptation serve as a "mind reading" tool?

Finally, the belief updating modelin this paper does not include any additional
information about what VOT distributions actually look like for a typical talker
of American English.  This was a deliberate choice, in order to see whether 

* Cognitive model of adaptation as belief updating allows us (potentially) to
  infer what listeners believe an unfamiliar talker will sound like by looking
  at patterns of adaptation or lack thereof across different distributions.
* model does recover prior beliefs that look SOMETHING like what we'd expect
  from production data...but not exactly
* and these beliefs DON'T generalize effectively to adaptation to different
  types of accents.
* ((( I guess this is just a reminder that interpretation of any computational
  model is contingent on all the assumptions that that model makes, and the kind
  of input data the model receives )))
