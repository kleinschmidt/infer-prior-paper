...
ex1_summary <- summary(b_logit_exp1)
...
fixef_tab <- ex1_summary$fixed %>%
  summary_to_tibble()

tabs <-
  ex1_summary$random %>%
  set_names(., glue("Random effects: {names(.)}")) %>%
  c(list("Fixed effects (β)"=ex1_summary$fixed), .) %>%
  map(summary_to_tibble)

kable(bind_rows(tabs),
      caption = "Bayesian mixed-effects logistic regression coefficients from Experiment 1") %>%
  pack_rows(index = map(tabs, nrow))
...
ex2_tabs <-
  ex2_summary$random %>%
  set_names(., glue("Random effects: {names(.)}")) %>%
  c(list("Fixed effects (β)"=ex2_summary$fixed), .) %>%
  map(summary_to_tibble)
...
ex2_tabs %>%
  bind_rows() %>%
  left_join(select(hyps_tbl, Coefficient=clean_beta, `BF (β=0)`)) %>%
  kable(caption = "Bayesian mixed-effects logistic regression coefficients from Experiment 2") %>%
  kable_styling(full_width = FALSE) %>%
  pack_rows(index = map(ex2_tabs, nrow))
