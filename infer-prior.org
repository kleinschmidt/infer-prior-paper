...
to broad shifts in multiple phonetic
    categories [@Maye2008a; @Babel2019; @Weatherholtz2015] or
...
*** distributional learning
    ...
    Second, both adults and infants are sensitive to the distributional
    properties of speech [@Maye2000; @Maye2002; @Feldman2013b]



 that
    both the number and acoustic-phonetic properties of many languages' phonetic
    categories can be learned from un-labeled observations of acoustic cues
    simply by distributional learning algorithms which extract the number and
    locations of clusters [].  On the other hand, rapid adaptation to already acquired
    categories can also be modeled as distributional learning, where a
    listener's beliefs about the current talker's cue distributions are adjusted
    incrementally to bring them into better alignment with the cues they have
    recently observed [@Kleinschmidt2015b; @
...
*** clear constraints on what adults can learn
    ...
    learning, what distinguishes them?  For one, it seems that distributional
    learning in adults is /constrained/.
    ...
    category in their native language [@Goto1971; @Myawaki1975].  Long-term
    naturalistic exposure is not sufficient to achieve good discrimination of
    this contrast, even after convserational competence has been achieved
    [@Takagi1995].  While perception of this contrast can be improved somewhat
    by training, it requries extensive training and these listeners seldom
    achieve native-like levels of performance [@Bradlow1997].
...
*** where do these constraints come from?
    if both acquisition and adaptation are forms of distributional learning,
    what's the difference between them?  maybe it's labels.  maybe it's priors
    (experience that leads you to expect to encounter particular kinds of
    distributions).
