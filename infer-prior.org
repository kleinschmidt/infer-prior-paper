...
** for revision
*** contrast between previous studies with adults
/Entered on/ [2020-10-30 Fri 11:18]

Previous distributional learning studies typically test for the existence of
/any/ distributional learning, using a binary contrast.  In order to understand
the extent and the nature of the constraints on distributional learning, we need
a more continuous, quanitative assessment of the /amount/ and /speed/ of
distributional learning.  Thus, instead of looking for any difference between
the categorization behavior of listeners exposed to two different sets of
acoustic cue distributions, in these studies I compare the categorization
functions across multiple different sets of distributions that vary in a
gradient way.
*** contrast between qualitative/quantitative explanations of acquisition
/Entered on/ [2020-11-02 Mon 14:18]

Can strengthen motivation by drawing a more explicit contrast between dist
learning as a meta-theory that links infant and adult language systems, contrast
that with theories that posit that there's some kind of qualitative difference.
The argument here is basically that the kinds of constraints we observe are
consistent with the continuous operation of dist learning at multiple levels,
but in order for this idea to have any teeth it needs to be testable, which is
what we're doing here.

another possible framing is to orient it in the separation of competence and
performance: adult language USE is of a different KIND than language
ACQUISITION.  contra that, we have these statistical/connectionist style
theories which argue that there's a continuous learning/updating that happens
(Chang et al.; Elman structure in time; etc.).  recent updates have suggested
that there's /higher order distributional learning/ as a possible explanation of
differences in flexibility...  but that connection is kind of just a promisory
note, an uncashed check...we need a way to test those explanations...
...
in intro, could have a "sources of constraints" section where I introduce
self-supervision (at the category/token level) and the distributions, including
the model (maybe in simplified form)

****** phonetically intermediate between aspirated and not..
look up the Cho paper on VOT and see if there are langauges that violate that
"partially aspirated" constraint (like Seoul Korean)?

****** variation in VOT across talkers
And the chodroff and wilson to get a sense of the variation across talkers
...
***** TODO clarify comparison with acquisition - why are they comparable??
One idea: emphasize in intro that acquisition is at least semi supervised
(contrary to original idea of role of distributional learning)

Actually, now that I've read this again, I think the discussion here could be
much better...unsupervised/supervised isn't quite the right opposition (either
to present or to explain...), rather, in the lack of explicit supervision, a
variety of sources of /self-supervision/ come into play, and we KNOW these are
relevant for acquisition of phonetic categories...but we don't necessarily know
quite what role they play in adaptation.  in a way, there's (at least) two kinds
of self-supervision that might operate: prior experience with other talkers
(supervision at the level of the distributions themselves), and the token
level.  in a way, what these results are showing is that the token level is not
what really matters here...it's the distributions themselves.

I think this framing plugs into the adaptation/acquisition comparison better
too, from the perspective of "pure" distributional learning experiments

the theme I'm starting to circle around now is "DL as bottom-up vs. top-down".

need to 'tell the story' of DL a bit more...as a theory of acquisition (I mean,
maye et al. 2000 is "learning phonemes without minimal pairs" for gotts sake)
and how it's been revised - minimal pairs might not be important but considering
higher-level context ("self-supervision" might be) - and in particular important
because they can separate two similar categories based on the context they
appear in - provides token/category level supervision signal (need an example
from Feldman?  or "happy/pet"

Is it also important for adults?  On the one hand, maybe it is: it's a very
useful piece of context.  On the other hand, other kinds of self-supervision
might be more informative...like experience with other talkers.  So we're
putting these head-to-head.  need a way to assess role of distributions, hasn't
really been done before systematically.
...
One approach here might be to put the theory up front more explicitly, show that
it can /capture/ qualitatively the pattern of results from ... what exactly I
don't know (the sumner paper?) ... and that it predicts constrained adaptation
...
putting the model first allows running simulations too - just put the sufficient
statistics (weighted by the posterior under the "standard" model) through to
show that you'd expect more learning.  actually, show it without a prior, just
the SS.
